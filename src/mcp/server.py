...
class SensorMCPToolService:
    ...
    DEFAULT_HUMIDITY_MIN = float(User.model_fields["humidity_min"].default)
    DEFAULT_HUMIDITY_MAX = float(User.model_fields["humidity_max"].default)
    ...
    async def get_current_reading(self) -> dict[str, Any]:
        """Return latest reading plus freshness metadata."""
        reading = await self.sensor_history_service.get_latest()

        if reading is None:
            return {
                "status": "no_data",
                "thresholds": self._serialize_default_thresholds(),
                "reading": None,
                "age_seconds": None,
                "is_stale": True,
            }
    ...
    @classmethod
    def _serialize_default_thresholds(cls) -> dict[str, float]:
        return {
            "humidity_min": cls.DEFAULT_HUMIDITY_MIN,
            "humidity_max": cls.DEFAULT_HUMIDITY_MAX,
        }
...
def create_mcp_server(config: Config) -> FastMCP:
    ...
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "Pass explicit chat_id only for threshold updates."
        ),
    ...
    @mcp.tool(description="Get latest sensor reading and freshness status.")
    async def get_current_reading() -> dict[str, Any]:
        return await _run_tool_with_logging(
            tool_name="get_current_reading",
            action=tools.get_current_reading,
        )

    @mcp.tool(description="Get recent sensor readings within a minute window.")
    async def get_recent_readings(
        minutes: int = 60,
        limit: int = 300,
    ) -> dict[str, Any]:
        return await _run_tool_with_logging(
            ...
            action=lambda: tools.get_recent_readings(minutes=minutes, limit=limit),
        )

    @mcp.tool(description="Update minimum humidity threshold for a specific chat_id.")
    ...
    @mcp.tool(description="Update maximum humidity threshold for a specific chat_id.")
...
async def _run_tool_with_logging(
    ...
    chat_id_log = chat_id if chat_id is not None else "-"
    try:
        result = await action()
        duration_ms = int((perf_counter() - started) * 1000)
        logger.info(
            "mcp_tool_success tool=%s chat_id=%s duration_ms=%s",
            tool_name,
            chat_id_log,
            duration_ms,
        )
